import sunImg from '../images/sun.png';
import mercuryImg from '../images/mercury.png';
import venusImg from '../images/venus.png';
import earthImg from '../images/earth.png';
import marsImg from '../images/mars.png';
import jupiterImg from '../images/jupiter.png';
import saturnImg from '../images/saturn.png';
import uranusImg from '../images/uranus.png';
import neptuneImg from '../images/neptune.png';

const objectsImages = [
  { name: 'Sol', image: sunImg },
  { name: 'Mercúrio', image: mercuryImg },
  { name: 'Vênus', image: venusImg },
  { name: 'Terra', image: earthImg },
  { name: 'Marte', image: marsImg },
  { name: 'Júpiter', image: jupiterImg },
  { name: 'Saturno', image: saturnImg },
  { name: 'Urano', image: uranusImg },
  { name: 'Netuno', image: neptuneImg },
];

export default objectsImages;
